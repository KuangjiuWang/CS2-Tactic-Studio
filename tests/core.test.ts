...
import { hasHlaeHookWarning } from '../src/hlae/diagnostics';
...
describe('Source2 capture generation',()=>{
 it('recognizes unresolved HLAE addresses as warnings that require render verification',()=>{expect(hasHlaeHookWarning('Could not find address for pattern: 48 89')).toBe(true);expect(hasHlaeHookWarning('Problem in C:\\_dev\\advancedfx\\AfxHookSource2\\addresses.cpp:120')).toBe(true);expect(hasHlaeHookWarning('AfxHookSource2 initialized')).toBe(false);});
 it('records native screen, audio and exact tick scheduling with no view reconstruction',()=>{const {project}=mockProject();const g=generateCapture({demoPath:'D:/demo.dem',directory:'D:/project/generated/job',steamId:'76561199141109937',startTick:8000,endTick:9000,tickRate:64,preferences:{...project.preferences,hlaePath:'D:/tools/HLAE.exe',cs2Path:'D:/CS2/cs2.exe'}});expect(g.cfg).toContain('HLAE 2.192.3 / AfxHookSource2 0.41.3; CS2 1.41.8.2 baseline.');expect(g.cfg).toContain('mirv_streams record startMovieWav 1');expect(g.cfg).toContain('mirv_streams record screen enabled 1');expect(g.script).toContain('spec_mode 1');expect(g.script).toContain('getEntityFromSplitScreenPlayer');expect(g.script).toContain('view.mode!==2');expect(g.script).toContain('addAtTick 8000');expect(g.script).toContain('getSteamId');expect(g.args.at(-1)).toContain('-insecure');expect(g.script).not.toContain('getRenderEyeOrigin');});
