...
def test_close_destroys_webview_before_waiting_for_backend():
    ...
    close_function = source[source.index("fn request_desktop_exit"):]
    destroy = close_function.index("window.destroy()")
    stop = close_function.index("stop_backend(&handle);", destroy)
    assert destroy < stop
