...
# The tactical batch observes real per-POV completions without creating a
# second CS2/OBS controller. ContextVar confines it to the calling request.
recording_result_observer: ContextVar = ContextVar("recording_result_observer", default=None)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="insight-player-aliases-") as alias_dir:
            ...
            results = await director.execute_plan_queue(
                ...
                fade_controller=fade_ctrl,
                on_result=recording_result_observer.get(),
            )
