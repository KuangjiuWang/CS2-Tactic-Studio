...
export default function PlaybookDialog({ dialog, folders, close, submit, busy }) {
  ...
  return <dialog ref={ref} className="playbook-dialog" onCancel={(e) => { e.preventDefault(); if (!busy) close(); }}><form onSubmit={(e) => { e.preventDefault(); submit({ name, selected, parent: parent || null, classification: { ...classification, tags: tagText.split(",").map((tag) => tag.trim()).filter(Boolean) } }); }}>
    ...
    {dialog.action === "delete" ? <p>{t(dialog.kind === "folders" ? "playbook.deleteFolderHint" : "playbook.deleteTacticHint")}</p>
      ...
      : isClassifier ? <div className="playbook-classification-form">
        ...
        <label>{t("playbook.customTags")}<input aria-label={t("playbook.customTags")} maxLength={500} value={tagText} onChange={(e) => setTagText(e.target.value)} placeholder={t("playbook.tagsHint")} /></label>
      </div>
    ...
    <footer><button type="button" disabled={busy} onClick={close}>{t("playbook.cancel")}</button><button className="playbook-primary" disabled={busy || (!isClassifier && !["delete", "move", "addToFolder"].includes(dialog.action) && !name.trim())}>{t(busy ? "playbook.saving" : "playbook.confirm")}</button></footer>
  </form></dialog>;
}
