...
export default function Demo2DReplayPreview({
  ...
  useEffect(() => {
    if (externalPlaying !== null) setPlaying(Boolean(externalPlaying));
  }, [externalPlaying]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!onPlayhead) return undefined;
    return playheadStoreRef.current.subscribe(() => {
      const snapshot = playheadStoreRef.current.getSnapshot();
      if (snapshot.tick > 0) onPlayhead(snapshot.tick);
    });
  }, [onPlayhead]);
