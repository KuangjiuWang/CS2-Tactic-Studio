...
- **自动录制全部玩家的雷达图** — 对局解析完成后，在 Demo 分析页点击「自动录制全部玩家雷达图」，即为本场全部玩家自动生成六维战力雷达图（KPR / 生存率 / ADR / KAST / 多杀回合 / Rating），参照 Rock-Radar-main 的霓虹雷达绘制风格，2560×1440（16:9）左右分割构图：左半部为雷达数据面板（雷达居中 + 背后 CNCS 水印），右半部为人物大肖像。蓝色外圈 = 最高刻度（满分基准：KPR 0.85 · 生存率 44% · ADR 85 · KAST 78% · 多杀 20% · Rating 1.3，仅发亮蓝线无填充），灰色内圈 = 等级区间，主题色多边形（青蓝 CT / 橙红 T）= 玩家实际数据，红色六边形 = 全场平均线（红描线 + 辉光，无填充）；超出满分的数值会溢出到蓝色外圈之外。
- **剪辑编排中插入** — 合辑工作台右侧控制台新增「cs数据图」专栏：确认「在剪辑前加入 CS 雷达图」后，从对局解析后的人物中选择一张雷达图，即可插入到指定片段之前，支持逐段调整时长与插入位置；导出成片时自动合成该图（有开场动画时使用动画视频段，否则用静态图）。
- **人物图片上传 / 队标** — 每张雷达图卡片都可通过前端接口上传人物图片（或一键套用 Steam 游戏内头像），上传后自动重新渲染；还可上传队伍标志，放大显示在人物头像后面（圆形裁切 + 径向羽化 + 半透明衬托）；未上传者使用游戏内头像或昵称首字占位。
- **慢入→快出→定格开场动画** — 每张卡片可一键生成开场动画视频（需已配置 FFmpeg）：个人数据多边形先缓速积蓄（慢入 Ease-in）→ 快速扩张到位（快出 Ease-out）→ 锁定定格（无过冲、无回弹），红色全场平均线全程可见；可多选卡片并发批量生成（帧渲染多进程并行，显著提速）。合辑工作台插入雷达段时自动使用动画视频段，面板内可预览。
