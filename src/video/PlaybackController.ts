...
export class PlaybackController {
 ...
 register(id:string,element:HTMLVideoElement,metadata:PovVideo,proxy=false){const map=proxy?this.proxies:this.videos;map.set(id,{element,metadata});element.muted=proxy;const ready=()=>this.sync(true);element.addEventListener('loadedmetadata',ready);this.sync(true);return()=>{element.removeEventListener('loadedmetadata',ready);element.pause();map.delete(id);};}
 ...
 const loop=(now:number)=>{const dt=this.last?Math.min((now-this.last)/1000,.1):0;this.last=now;const s=useStore.getState();
  if(s.playing&&s.viewMode==='tactical'){this.fractionalTick+=dt*s.match.tickRate*s.playbackRate;const ticks=Math.floor(this.fractionalTick);if(ticks){s.seek(s.currentTick+ticks);this.fractionalTick-=ticks;}}
 ...
 sync(force=false){const s=useStore.getState();this.stopFrame();const active=s.viewMode==='pov'?this.videos.get(s.selectedPlayer):undefined;
  ...
  const ready=()=>{if(generation!==this.generation)return;this.switching=false;if(useStore.getState().playing){v.play().then(()=>this.watch(v,m)).catch(e=>{useStore.getState().set({playing:false,error:`POV playback failed: ${e.message}`});});}else v.pause();};
  if(v.readyState>=1&&(force||Math.abs(v.currentTime-wanted)>.1)){this.switching=true;v.pause();if(Math.abs(v.currentTime-wanted)<.001){ready();}else{v.addEventListener('seeked',ready,{once:true});v.currentTime=Math.max(0,wanted);}}else ready();
 }
