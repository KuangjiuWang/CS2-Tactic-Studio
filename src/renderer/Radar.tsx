...
import type { Drawing,MapOverview,PlayerFrame } from '../types';
const utilityLabels:Record<string,string>={smoke:'S',flash:'F',he:'HE',molotov:'M',decoy:'D',c4:'C4'};
function drawObject(ctx:CanvasRenderingContext2D,d:Drawing,size:number,selected=false){
 ...
 else if(d.kind==='text'){ctx.font='600 14px Segoe UI';ctx.fillStyle='#0b111b';ctx.fillRect(x-5,y-18,ctx.measureText(d.text??'Note').width+14,26);ctx.fillStyle=d.color;ctx.fillText(d.text??'Note',x,y);}
...
export function Radar(){
 ...
 useEffect(()=>{
  ...
  const render=()=>{if(!alive)return;const rect=host.current!.getBoundingClientRect();const size=Math.min(rect.width,rect.height),dpr=window.devicePixelRatio||1;if(canvas.width!==Math.round(size*dpr)){canvas.width=Math.round(size*dpr);canvas.height=Math.round(size*dpr);canvas.style.width=canvas.style.height=`${size}px`;}
   ...
   ctx.textAlign='left';for(const d of step.annotations){const view=drag.current?.id===d.id?draft.current??d:d;drawObject(ctx,view,size,s.selectedObject===d.id);}if(draft.current&&!drag.current)drawObject(ctx,draft.current,size);
 ...
 if(!map)return <div className="empty"><h2>Unsupported map: {mapName}</h2><p>Import a calibrated overview JSON in Preferences.</p></div>;
 return <div className="radar-host" ref={host}><canvas aria-label="Tactical map" ref={ref} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{draft.current=null;drag.current=null;playerDrag.current=null;}} onWheel={e=>{if(!e.shiftKey)return;const s=useStore.getState();if(!s.editing)return;const rect=ref.current!.getBoundingClientRect(),x=(e.clientX-rect.left)/rect.width,y=(e.clientY-rect.top)/rect.height;s.editStep(step=>({...step,players:step.players.map(p=>{const [u,v]=worldToRadar(map,p.x,p.y,p.z);return Math.hypot(u-x,v-y)<.03?{...p,yaw:p.yaw+Math.sign(e.deltaY)*10}:p;})}));}}/>{imageError&&<div className="map-error">Radar image missing. Import an overview in Preferences.</div>}{note&&<form className="note-editor" onSubmit={e=>{e.preventDefault();if(note.value.trim())useStore.getState().addDrawing({id:crypto.randomUUID(),kind:'text',color:useStore.getState().color,points:[note.position],text:note.value});setNote(null);}}><input aria-label="Note text" autoFocus value={note.value} placeholder="Write a tactical note…" onChange={e=>setNote({...note,value:e.target.value})}/><button type="submit">Add note</button><button type="button" onClick={()=>setNote(null)}>Cancel</button></form>}</div>;
}
